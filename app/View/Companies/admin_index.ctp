	...
	<div class="box-body" id="table-list">
		<?php echo $this->element('flash-message'); ?>
			...
			$option['col'] =array(
			  ...
			  3 => array('key_tab' =>'updated','title_tab' => '最終更新','option_tab' => 'sort'),
			  4 => array('key_tab' =>'','title_tab' => '操作','option_tab' => ''),
			);
...
//        pr($companies);die;
        foreach ($companies as $key => $company): ?>
			<tr>
		        ...
		        <td><?php echo h(date('Y-m-d H:i:s',strtotime($company['Company']['updated']))); ?>&nbsp;</td>
				<td class="actions">
		            ...
		            <a href="/admin/companies/delete/<?= $company['Company']['id']?>" id="post-delete" class="btn btn-danger btn-sm btn-cat-cancel" title="削除する" onclick="check_delete('/admin/companies/delete/'+<?= $company['Company']['id']?>, event)"><i class="glyphicon glyphicon-remove icon-white"></i></a>
				</td>
			</tr>
		<?php endforeach; 
