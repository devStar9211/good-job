...
class AclController extends AclManagerAppController {
	...
	public function admin_permissions() {
		$this->set('title_for_layout', __('Quyền xem'));
		// Saving permissions
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			foreach ($perms as $aco => $aros) {
				...
				foreach ($aros as $node => $perm) {
					...
					if ($action == 'controllers/Frontend/list_post') {
						if ($perm == 'allow') {
							$this->Acl->allow($node, $action);
							$this->Acl->allow($node, 'controllers/Frontend/post_view');
							$this->Acl->allow($node, 'controllers/Frontend/loadPost');
						}
						elseif ($perm == 'deny') {
							$this->Acl->deny($node, $action);
							$this->Acl->deny($node, 'controllers/Frontend/post_view');
							$this->Acl->deny($node, 'controllers/Frontend/loadPost');
						}
					}else if($action == 'controllers/Frontend/daily_settlement'){
						if ($perm == 'allow') {
							$this->Acl->allow($node, $action);
							$this->Acl->allow($node, 'controllers/Frontend/get_daily_data');
						}
						elseif ($perm == 'deny') {
							$this->Acl->deny($node, $action);
							$this->Acl->deny($node, 'controllers/Frontend/get_daily_data');
						}
					}else if($action == 'controllers/Frontend/budget_ranking'){
						if ($perm == 'allow') {
							$this->Acl->allow($node, $action);
							$this->Acl->allow($node, 'controllers/Frontend/get_budget_ranking_data');
						}
						elseif ($perm == 'deny') {
							$this->Acl->deny($node, $action);
							$this->Acl->deny($node, 'controllers/Frontend/get_budget_ranking_data');
						}
					}else if($action == 'controllers/Frontend/budget_sale'){
						if ($perm == 'allow') {
							$this->Acl->allow($node, $action);
							$this->Acl->allow($node, 'controllers/Frontend/get_last_year_comparison_data');
						}
						elseif ($perm == 'deny') {
							$this->Acl->deny($node, $action);
							$this->Acl->deny($node, 'controllers/Frontend/get_last_year_comparison_data');
						}
					}else if($action == 'controllers/Frontend/user_page'){
						if ($perm == 'allow') {
							$this->Acl->allow($node, $action);
							$this->Acl->allow($node, 'controllers/Frontend/get_data_prize');
						}
						elseif ($perm == 'deny') {
							$this->Acl->deny($node, $action);
							$this->Acl->deny($node, 'controllers/Frontend/get_data_prize');
						}
					}else{
						if ($perm == 'allow') {
							$this->Acl->allow($node, $action);
			...
			return $this->redirect(array('action' => 'permissions'));
		}
		 ...
		 */
		$parent_id = $this->Acl->Aco->find('list', array(
			'fields' => array('id'),
			'conditions'=>array(
				'Aco.alias'=>Configure::read('Role.alias'),
			))
		);

		$this->acos = $acos = $this->Acl->Aco->find('all', array('conditions' => array('Aco.parent_id' => current($parent_id)), 'order' => 'Aco.lft ASC', 'recursive' => 1));
		$perms = array();
		$parents = array();
	 ...
	 */
	private function _evaluate_full_permissions($permKeys, $aro, $aco, $aco_index) {
		...
		if ($aco['Aco']['parent_id'] == null) {
			...
			$evaluate = $this->_evaluate_full_permissions($permKeys, $aro, $parent_aco, $key);
	...
	private function _evaluate_permissions($permKeys, $aro, $aco, $aco_index) {
		...
		if (count($allowedPerms) === count($permKeys)) {
			...
			if ($aco['Aco']['parent_id'] == current($parent_id)){
				$this->lookup +=1;
				$acoNode = (isset($aco['Action'])) ? $aco['Action'] : null;
				$aroNode = array('model' => $aro['alias'], 'foreign_key' => $aro['id']);
				$allowed = $this->Acl->check($aroNode, $acoNode);
	...
	public function admin_full_permissions() {
		...
		foreach ($acos as $key => $data) {
			...
			foreach($aros as $aro) {
				$aroId = $aro[$Aro->alias][$Aro->primaryKey];
				$evaluate = $this->_evaluate_full_permissions($permKeys, array('id' => $aroId, 'alias' => $Aro->alias), $aco, $key);
