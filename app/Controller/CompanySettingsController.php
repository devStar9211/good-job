...
class CompanySettingsController extends WidgetController
{
    public function admin_index($id = null) {
        $this->set("title_for_layout",__('Company Setting'));
        $model = 'CompanySetting';
        $controller = 'company_settings';
        $this->uses[] = $model;
        if(!empty($this->data)){
        ...
        $this->render('/CompanySettings/admin_index');
    }
        ...
        try {
            if (!empty($model) && !empty($postdata) ) {
                ...
                if($postdata['EditData']['id'] > 0){
                    ...
                    $dataSave['color']= $postdata['FormData']['color'][$postdata['EditData']['id']];
                }else{
                    $dataSave['id']= '';
                    $dataSave['name']= trim($postdata['EditData']['name']);
                    $dataSave['color']= $postdata['EditData']['color'];
                }
                $this->{$model}->create();
