...
class AllowancesController extends WidgetController
{
	public function admin_index($id = null) {

        $this->set("title_for_layout",__('Đăng ký trợ cấp DB'));
        $model = 'Allowance';
        ...
        try {
            if (!empty($model) && !empty($postdata) ) {
                ...
                if($postdata['EditData']['id'] > 0){
                    ...
                    $dataSave['option']= $postdata['FormData']['option'][$postdata['EditData']['id']];
                }else{
                    $dataSave['id']= '';
                    $dataSave['name']= trim($postdata['EditData']['name']);
                    $dataSave['price']= $postdata['EditData']['price'];
                    $dataSave['option']= $postdata['EditData']['option'];
                }
                    $this->{$model}->create();
